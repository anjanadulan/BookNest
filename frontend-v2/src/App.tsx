...
function App() {
  const [view, setView] = useState<"home" | "catalog" | "details" | "auth" | "cart" | "checkout" | "payment" | "orders">("home")
  const [selectedBookId, setSelectedBookId] = useState<number | null>(null)
  const [authReturnView, setAuthReturnView] = useState<"home" | "cart">("home")
  ...
  const [orders, setOrders] = useState<OrderRecord[]>([
    {
      ...
      total: 43.99,
      status: "Completed",
      items: [{ bookId: 2, quantity: 1 }, { bookId: 4, quantity: 1 }],
  ...
  function addToCart(bookId: number, quantity = 1) {
    setCartItems((current) => {
      ...
      if (existing) {
        return current.map((item) => item.bookId === bookId ? { ...item, quantity: item.quantity + quantity } : item)
      }
  ...
  function changeCartQuantity(bookId: number, quantity: number) {
    ...
    setCartItems((current) => current.map((item) => item.bookId === bookId ? { ...item, quantity } : item))
  }
  ...
  function openAuth(returnTo: "home" | "cart" = "home") {
  ...
  function completePayment(payment: PaymentResult) {
    ...
    setOrders((current) => [{
      ...
      status: "Completed",
      items: cartItems,
      paymentMethod: payment.paymentMethod === "CREDIT_CARD" ? `Card ending ${payment.lastFour}` : payment.paymentMethod,
  ...
  if (view === "details" && selectedBookId !== null) {
    ...
    if (selectedBook) {
      return (
        <BookDetailsPage
          ...
          onBack={openCatalog}
          onOpenCart={openCart}
          onAddToCart={(quantity) => addToCart(selectedBook.id, quantity)}
          savedBooks={savedBooks}
          onToggleSaved={toggleSaved}
          onSelectBook={openDetails}
        />
  ...
  if (view === "auth") {
    return <AuthPage onBack={() => setView(authReturnView)} onSuccess={finishAuth} />
  }

  if (view === "cart") {
    return <CartPage cartItems={cartItems} cartCount={cartCount} isAuthenticated={Boolean(user)} userName={user?.name} onBack={openCatalog} onContinueShopping={openCatalog} onChangeQuantity={changeCartQuantity} onRemove={removeCartItem} onCheckout={handleCheckout} />
  }

  if (view === "checkout" && user && cartItems.length > 0) {
    return <CheckoutPage cartItems={cartItems} user={user} onBack={openCart} onContinueToPayment={continueToPayment} />
  }

  if (view === "payment" && checkoutDetails && cartItems.length > 0) {
    return <PaymentPage cartItems={cartItems} details={checkoutDetails} onBack={() => setView("checkout")} onComplete={completePayment} />
  }

  if (view === "orders") {
    return <OrderHistoryPage orders={orders} onBack={returnHome} onContinueShopping={openCatalog} />
  }
      ...
      <header className="relative z-20 mx-auto flex h-[70px] max-w-[1360px] items-center justify-between border-b border-line px-6 md:h-[82px] md:px-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            variant="ghost"
            type="button"
            aria-label={user ? "View order history" : "Open account"}
            onClick={() => user ? openOrders() : openAuth("home")}
          >
      ...
      >
        <div className="animate-[reveal-in_800ms_cubic-bezier(.23,1,.32,1)_90ms_both] self-center md:pl-[4vw]">
          ...
          <p className="mt-6 max-w-[405px] text-base leading-[1.65] text-muted">
            Thoughtful books for curious minds. Find the ideas you need, the
            stories you will keep, and a little more room to think.
          </p>
          ...
          <div className="group relative h-full rotate-[2.5deg] overflow-hidden transition-transform duration-500 ease-out hover:rotate-0">
            ...
            <div className="absolute inset-x-7 bottom-7 flex items-end justify-between">
              <div>
                ...
                <p className="mt-2 text-xs text-ink/70">F. Scott Fitzgerald</p>
              </div>
              <Button
      ...
      >
        <div className="mx-auto mb-10 flex max-w-[1160px] flex-col items-start gap-6 md:mb-14 md:flex-row md:items-end md:justify-between">
          ...
          <p className="mb-1 max-w-[275px] text-sm leading-[1.65] text-muted">
            No endless shelves. Just books with something to say, selected for
      ...
      >
        <div className="pt-2">
          ...
          <p className="mt-7 max-w-[210px] text-sm leading-[1.6] text-muted">
            Because your reading list should feel like a place you want to be.
          </p>
        </div>
        <div className="flex overflow-x-auto pb-2 md:grid md:grid-cols-3 md:overflow-visible">
          {[
            ...
            <div
              ...
              key={number}
            >
              <span className="font-mono text-[9px] opacity-55">{number}</span>
